...
<html>
	<head>
		...
		<script type="text/javascript">
			function delFromProductCart(pid){
				if(confirm("您要删除该商品？")){
					location.href="${pageContext.request.contextPath }/product?method=delFromProductCart&pid="+pid;
				}
			}
				...
				<div style="margin:0 auto; margin-top:10px;width:950px;">
					...
					<table class="table table-bordered">
						<tbody>
							...
							<c:forEach items="${cart.cartItemList }" var="entry">
							<tr class="active">
								...
								<td width="10%"><input type="text" name="quantity"
									value="${entry.value.count}" maxlength="4" size="10"></td>
								<td width="15%"><span class="subtotal">￥${entry.value.subtotal}</span></td>
								<td><a href="javascript:;" onclick="delFromProductCart('${entry.value.product.pid}')" class="delete">删除</a></td>
							</tr>
						</c:forEach>
